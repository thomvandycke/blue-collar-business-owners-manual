...
export function UserAvatarChip({ name, imageUrl, className }: UserAvatarChipProps) {
  return (
    <div className={cn("inline-flex items-center gap-2", className)}>
      <div className="relative h-7 w-7 overflow-hidden rounded-full bg-slate-200 text-[11px] font-semibold text-slate-700">
        {imageUrl ? (
      ...
      <span className="text-sm text-slate-700">{name ?? "Unassigned"}</span>
    </div>
  );
}
